...
Сайт статичен и берёт данные из БД через API. Чтобы залить
обновлённый список (исправленные роли, годы рождения):

```bash
# 1. Локально — пересобрать result_fixed.json
python fix_result.py result.json result_fixed.json

# 2. Загрузить result_fixed.json и import_to_db.py на сервер, затем:
python3 import_to_db.py result_fixed.json --reset
```

⚠️ Флаг `--reset` обязателен при повторном импорте: без него
существующие люди пропускаются как дубли и обновлённые данные
в базу не попадут. `--reset` очищает только `persons` и
`service_records` (дивизия, подразделения, события не трогаются).

PostgREST отдаёт данные вживую — перезапуск не нужен. В браузере
сделать жёсткое обновление (Ctrl+F5).
